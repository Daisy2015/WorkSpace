...
interface IntelligentChartTopBarProps {
  lang: 'zh' | 'en';
...
export const IntelligentChartTopBar: React.FC<IntelligentChartTopBarProps> = ({
  lang,
  activeWorkspaceData,
  currentUser,
  onBackToList,
  ...
  return (
    <nav className="relative h-16 bg-white border-b border-slate-200 flex items-center px-6 justify-between flex-shrink-0 z-40 shadow-sm select-none" id="intelligent-chart-top-bar">
      ...
      <div className="flex items-center gap-4 flex-shrink-0">
        ...
        <div className="flex flex-col min-w-[120px] max-w-[240px]">
          <div className="flex items-center gap-1.5 group">
            <span className="font-bold text-slate-900 tracking-tight truncate max-w-[150px]" id="workspace-detail-name-chart" title={activeWorkspaceData?.name}>
            ...
            {activeWorkspaceData?.owner === currentUser && (
              <button 
                ...
                id="btn-edit-workspace-name-chart"
              >
                <i className="fas fa-edit text-xs"></i>
              </button>
            )}
            <span className="px-1.5 py-0.5 rounded bg-blue-50 text-[10px] font-bold text-blue-600 uppercase tracking-wider border border-blue-100" id="workspace-detail-status-chart">
              {activeWorkspaceData?.status || 'DRAFT'}
            </span>
          </div>
          {activeWorkspaceData?.description && (
            <span className="text-[10px] text-slate-400 font-medium truncate max-w-[220px]" id="workspace-detail-desc-chart" title={activeWorkspaceData.description}>
              {activeWorkspaceData.description}
            </span>
          )}
        </div>
      ...
      {/* CENTER SECTION: Active Agent Runtime Panel (Always Running, No Close/Exit button) */}
      <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 flex items-center gap-3 bg-slate-50/95 border border-slate-200/60 rounded-xl px-3 py-1 shadow-xs z-10">
        ...
        <div className="flex items-center gap-2 pr-2.5 border-r border-slate-200 flex-shrink-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <h2 className="text-xs font-bold text-slate-800 leading-none truncate max-w-[120px]">{lang === 'zh' ? '专业成图智能体' : 'Pro Mapping Agent'}</h2>
              <div className="flex items-center gap-0.5 px-1 py-0.5 border bg-indigo-50 border-indigo-100 text-indigo-600 rounded-full flex-shrink-0 scale-90 origin-left">
                ...
                <span className="text-[7px] font-black uppercase tracking-[0.03em] text-indigo-600">{lang === 'zh' ? '智能成图...' : 'AI MAPPING...'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex-shrink-0 flex items-center gap-1.5 pl-0.5">
          ...
          <button 
            ...
            id="btn-top-bar-well-dropdown-chart"
          >
        ...
        <div className="relative flex-shrink-0 flex items-center gap-1.5">
          ...
          <button 
            ...
            id="btn-top-bar-tpl-dropdown-chart"
          >
        ...
        {/* Integrated Actions for Agent: Assistant & Exit (WITHOUT exit close button) */}
        ...
        {onToggleAssistant && (
          <button 
            ...
            id="btn-top-bar-toggle-assistant-integrated-chart"
          >
      ...
      <div className="flex items-center gap-4 flex-shrink-0">
        <div className="flex items-center gap-1 text-slate-400">
          ...
          <button 
            ...
            id="btn-workspace-settings-chart"
          >
